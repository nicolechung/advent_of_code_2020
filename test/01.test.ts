import { pairs, threes, multiply, numbers, answer, followUp } from '../src/01';

describe('Day one tests', () => {
  it('pairs finds two numbers that add up to 2020', () => {
    expect(pairs([1721, 979, 366, 299, 675, 1456])).toEqual([1721, 299]);
  });
  it('threes finds three numbers that sum to 2020', () => {
    expect(threes([1721, 979, 366, 299, 675, 1456])).toEqual([979, 366, 675]);
  });
  it('threes returns empty array for numbers that do not sum to 2020', () => {
    expect(threes([1, 2, 3, 4, 5])).toEqual([0, 0, 0]);
  });
